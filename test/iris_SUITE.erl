...
init_per_suite(Config) ->
    ...
    DisplayRow = fun
        ({newline, [A, B, C, D, Class]}, Acc) ->
            [
                {[list_to_float(A), list_to_float(B), list_to_float(C), list_to_float(D)],
                    list_to_binary(Class)}
                | Acc
            ];
        (_, Acc) ->
            Acc
...
train_random_supervised(Config) ->
    Samples = ?config(rows, Config),
    {ok, SOM} = som:new(10, 10, 4, false, #{
        classes => #{<<"setosa">> => 0.0, <<"verisicolor">> => 0.0, <<"virginica">> => 0.0}
    }),
    ...
    ct:pal("matched ~p/~p => ~p%", [
        Matched,
        length(Unsupervised),
        Matched / length(Unsupervised) * 100
    ]),
    %% TODO: add real asserts here
    ?assert(true).
